---
title: "Transcriptomic analyses of GmPR-1 genes"
...
## Transcriptomic analysis of GmPR-1 genes
...
First of all, we all observe the global expression profiles of GmPR-1 genes to identify genes that are broadly expressed (likely to have endogenous functions) and genes that are only activated in response to stress.

```{r id_conversion}
# Load table of ID conversion
library(here)
ids <- read.csv(here("products", "tables", "table1.txt"))[, 1:2]
```
...
metadata1 <- read.csv(here("data", "atlas_metadata_stress_samples.tsv"), 
                      header = TRUE, sep="\t")[, c(5,7,1)]
metadata2 <- read.csv(here("data", "newsamples_metadata_stress_samples.tsv"),
                      header=TRUE, sep="\t")[, c(5,7,1)]
...
ht_opt(
  ...
  message=FALSE)
cheatmap_global <- pheatmap(globalexp, 
                            ...
                            border_color=NA,
                            fontsize_row = 7,
                            main="Global expression profile of GmPR-1 genes")
cheatmap_global_ggplot <- ggplotify::as.ggplot(cheatmap_global, scale=0.95)

ggplot2::ggsave(file=here("products", "plots", "heatmap_globalexpression.png"),
...
load(here("products", "result_files", "tree_clusters.rda"))
gene_order_tree <- gsub("\\|.*", "", gene_order_tree)

# Create data frame containing Tau indices
tau_pr1 <- data.frame(Gene=rownames(globalexp), 
...
clusters <- data.frame(Gene=gene_order_tree,
                       Cluster=c(rep("C2", 12), rep("C1", 13)),
                       stringsAsFactors = FALSE)[-1,] 

# Combine data frames
tau_plotdata <- merge(tau_pr1, clusters)
library(ggplot2)
tau_violin <- ggpubr::ggviolin(tau_plotdata, x="Cluster", y="Tau", 
                               ylim=c(0,1),
                               trim=TRUE,
                               fill="Cluster", palette="npg", legend="none",
...
#----Export figure----
ggsave(tau_violin, 
       file=here("products", "plots", "Tau_violin.pdf"),
       width=5, height=5)
```


## Analysis of differentially expressed PR-1 genes

Let's run the previously defined functions to find differentially expressed GmPR-1 genes.

```{r dge_prep, message=TRUE}
...
save(abiotic_hm_plotdata, biotic_hm_plotdata, 
     ...
     file = here("products", "result_files", "dge_objects.RData"),
     compress="xz")
```
...
```{r}
load(here("products", "result_files", "dge_objects.RData"))
...
ht_opt(
  ...
  legend_title_gp = gpar(fontsize = 8, fontface = "bold"),
  message=FALSE)
cheatmap_abiotic <- pheatmap(abiotic_hm_plotdata, 
                             ...
                             border_color=NA,
                             fontsize_row = 8,
                             main="GmPR-1 response to abiotic stress")
cheatmap_abiotic_ggplot <- ggplotify::as.ggplot(cheatmap_abiotic, scale=0.95)

#----Plot heatmap of fold change in response to abiotic stress----
cheatmap_biotic <- ComplexHeatmap::pheatmap(biotic_hm_plotdata, 
                                            ...
                                            border_color=NA,
                                            fontsize_row = 8,
                                            main="GmPR-1 response to biotic stress"
                                            )
...
library(ggplot2)
ggsave(
    filename = here("products", "plots", "Fig3_expression.png"), 
    exp_figure,
    width=11, height=11,
    dpi=300
)
...
readr::write_tsv(contrast_export, 
                 file = here("products", "tables", "contrast_table.tsv"))

readr::write_tsv(sample_export, 
                 file = here("products", "tables", "stress_sample_table.tsv"))
```
